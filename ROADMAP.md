...
この時点では、まだ本格的なアセンブラは作らない。`tools/small-asm.c` は `MOVI`, `SYSCALL`, `HALT`, `CMP`, `JUMP`, `JZ`, `JNZ` だけを扱う最小ツールとして育てる。

次は、起動メッセージや `help` 表示に向けて、文字列表示とデータ配置をOS風プログラム側で扱いやすくする。
