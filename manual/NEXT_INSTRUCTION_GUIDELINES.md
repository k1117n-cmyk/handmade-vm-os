...
この文書は、次の命令やVM機能をどの順番で追加するかを整理するためのガイドラインです。
...
現在は Day 26 `prompt-echo` サンプルまで進んでいる。

次の節目は、1文字コマンド判定へ進むために、必要になった命令だけ `small-asm` へ追加すること。

```text
Day 27候補: small assemblerにMOV / CMP / JUMP / JZ / JNZを必要に応じて追加する
```

最初の成功条件:

```text
h / q / その他を判定する外部プログラムを書ける準備ができる
```

この段階では、いきなり行編集やOS風コマンドループへ進まない。
