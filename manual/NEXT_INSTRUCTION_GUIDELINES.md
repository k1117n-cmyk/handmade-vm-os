...
この文書は、Day 15 `POP` まで進んだ後に、次の命令やVM機能をどの順番で追加するかを整理するためのガイドラインです。
...
Day 16 から Day 20 までは、ブログ記事とのつながりを優先する。
...
現在は Day 23 `small assembler` まで進んでいる。

次の節目は、命令値を手で書く段階から、アセンブリ表記の小さなソースを外部バイナリへ変換する段階まで進んだうえで、OS風プログラムに必要な入力系へ進むこと。

```text
Day 21: 外部バイナリローダー
Day 22: hello.bin 作成ツール
Day 23: small assembler
Day 24候補: 入力系 SYSCALL
```
...
この段階では、本格的なアセンブラやOS起動へ進まない。手で作れる小さなバイナリを読み込み、最小アセンブリから生成できればよい。
...
Day 20: CALLI / RET
Day 21: 外部バイナリローダー
Day 22以降: 自作OSで必要になった命令やsyscallを追加
```
