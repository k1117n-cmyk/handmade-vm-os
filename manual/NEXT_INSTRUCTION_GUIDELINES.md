...
現在は Day 28 `command-loop` サンプルまで進んでいる。

次の節目は、起動メッセージや `help` 表示へ進むために、文字列表示とデータ配置をOS風プログラム側で扱いやすくすること。

```text
Day 29候補: 起動メッセージとhelp表示に向けて文字列表示を整理する
```
...
この段階では、いきなり行編集へ進まない。まずは1文字コマンドループに表示メッセージを足す。
